...
- `review_application`: generated by `apply-review`; do not hand-edit unless debugging an applied rewrite.
...
- Keep citation-bearing chunks narrow: one citation should support the immediately preceding claim. This also lets `apply-review` safely replace weak citation-bearing text with safer wording.
...
- Use `python3 -m citationtool.cli apply-review <spec.json> --verification <artifact-dir>/reference_verification.json --build` to create a separate reviewed spec and draft from weak-claim suggestions.
